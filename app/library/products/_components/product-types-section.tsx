...
export function ProductTypesSection({ types }: { types: ProductType[] }) {
  ...
  return (
    <CollapsibleSection
      title={t.library.products.types.title}
      subtitle={t.library.products.types.subtitle}
      defaultOpen={types.length === 0}
    >
      {types.length === 0 ? (
        <EmptyState message={t.library.products.types.empty} />
      ) : (
            ...
            <LibraryChip
              ...
              onDelete={() =>
                confirmToast({
                  ...
                  onConfirm: () =>
                    withDelete({
                      action: () => deleteProductType(type.id),
                      start: startDelete,
                      queryClient,
                      invalidateKeys: [["product-types"]],
