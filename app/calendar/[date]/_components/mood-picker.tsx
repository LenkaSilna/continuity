...
export function MoodPicker({
  ...
  const [optimisticMood, setOptimisticMood] = useOptimistic(mood);
  ...
  return (
    <section className="space-y-2 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
      ...
      <div className="flex flex-wrap gap-2">
        {levels.map((level) => {
          const active = optimisticMood === level;
          return (
            <button
              key={level}
              type="button"
        ...
        <button
          type="button"
          disabled={isPending || optimisticMood === null}
          onClick={() =>
            start(async () => {
              setOptimisticMood(null);
              const result = await setMood(date, null);
              if (result?.error) showError(t.calendar.errors.generic);
            })
          }
