...
- **Netlify** — deploy
- **PWA** — `manifest.json` + `display: standalone` → iPhone home screen + service worker (static cache)
...
2. Copy env example and fill in values:
   ...
   | `ALLOWED_EMAILS` | Comma-separated emails allowed to sign in. **Empty = open in dev, blocked in production.** |
   | `AI_API_KEY` | API key for the AI provider (used server-side only) |
...
When `ALLOWED_EMAILS` is empty: open in development, **blocked in production**.

## Project structure

```text
app/
  layout.tsx                  ← PWA meta + I18nProvider + theme + accent (cookie-based, no DB)
  ...
  _components/
    ...
    add-dashed-button.tsx     ← full-width dashed "add" button (reusable)
    page-skeleton.tsx         ← shared loading skeleton (used by all loading.tsx)
  _actions/login.ts           ← server action with allowlist check
  auth/
    callback/route.ts         ← OTP exchange + allowlist + appearance cookie seeding
  ...
  ai/
    page.tsx                  ← tabs: Výchozí (predefined) / Moje prompty (custom)
    [type]/page.tsx           ← predefined prompt — generate, edit, save override, restore
    _components/
      ai-tabs.tsx             ← tab switcher + custom prompt list
      prompt-editor.tsx       ← editable textarea + copy/regenerate/save/restore
    _actions.ts               ← savePromptOverride / deletePromptOverride
    custom/
      new/page.tsx            ← create custom prompt
      [id]/page.tsx           ← run custom prompt
      [id]/edit/page.tsx      ← edit / delete custom prompt
      _actions.ts             ← createCustomPrompt / updateCustomPrompt / deleteCustomPrompt
      _components/
        custom-prompt-form.tsx ← name + question + data block chip selector
lib/
  ...
  appearance.ts               ← cookie names, max-age, parse helpers, appearanceCookieOpts()
  ai-prompts.ts               ← server-side prompt builders (4 predefined + buildCustomPrompt)
...
public/
  sw.js                       ← service worker: cache-first for _next/static/, network-only for rest
  manifest.json + icons
supabase/migrations/
...
- [x] AI prompt generator — 4 predefined types (skincare, supplements, correlation, weekly)
      with routine context, data window info, editable textarea, copy/regenerate
...
- [x] Performance — cookie-based theme/accent (no DB on layout critical path),
      service worker, shared PageSkeleton for all loading states
- [x] Security — open redirect fix, allowlist blocks in prod, httpOnly cookies,
      explicit auth checks (defense in depth) in all server actions
